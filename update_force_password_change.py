...
try:
    ...
    if not users:
        print("✓ All officers already have force_password_change enabled or no officers found")
    else:
        print(f"Found {len(users)} officers that need to change password on first login:\n")
        ...
        for user_id, username, role in users:
            cursor.execute("""
                ...
                WHERE id = %s
            """, (user_id,))
            print(f"  ✓ {username} (ID: {user_id}) - will require password change on next login")

...
try:
    ...
    users_response = client.table('users').select('id, username, email, created_at').limit(3).execute()
    ...
    cases_response = client.table('cases').select('id, case_number, status, created_at').limit(3).execute()
    cases = cases_response.data
    
    if cases:
        print(f"✅ Successfully pulled {len(cases)} cases:")
        for case in cases:
            print(f"   - ID: {case.get('id')}, Case#: {case.get('case_number')}, Status: {case.get('status')}")
    else:
        print("⚠️  No cases found")
    
    # Pull activity logs
    print("\n📥 Pulling 'activity_logs' table from Supabase...")
    logs_response = client.table('activity_logs').select('id, action, user_id, created_at').order('created_at', desc=True).limit(5).execute()
    logs = logs_response.data
    
    if logs:
        print(f"✅ Successfully pulled {len(logs)} activity logs:")
        for log in logs:
            print(f"   - Action: {log.get('action')}, User: {log.get('user_id')}, Time: {log.get('created_at')}")
    else:
...
try:
    ...
    with app.app_context():
        if supabase_sync and supabase_sync.client:
            ...
            print(f"   Client: {'✓ Connected' if supabase_sync.client else '✗ Not connected'}")
            ...
            print(f"   Last Sync Times: {len(supabase_sync.last_sync_times)} tables synced")
            
            # Show last sync times
            if supabase_sync.last_sync_times:
                print("\n   Last sync operations:")
                for table, timestamp in list(supabase_sync.last_sync_times.items())[-3:]:

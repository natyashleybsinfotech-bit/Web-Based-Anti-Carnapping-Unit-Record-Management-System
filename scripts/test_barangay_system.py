...
def test_hotspot_calculation():
        ...
        *[
            {
                "id": i,
                "barangay_number": 10,
                "incident_date": f"2026-04-{i:02d}",
                "incident_location": "Tondo West",
                "status": "closed",
            }
            for i in range(1, 13)
        ],
    ...
    if result["hotspots"]:
        for hotspot in result["hotspots"]:
            risk_icon = (
                "🔴"
                if hotspot["status"] == "high_risk"
                else "🟠" if hotspot["status"] == "medium_risk" else "🔵"
            )
            print(
                f"{risk_icon} Barangay {hotspot['barangay_number']:3d} | {hotspot['district_name']:35s} | {hotspot['incident_count']} incidents"
            )
    else:
...
def test_manila_districts():
    ...
    for district in districts:
        print(
            f"District {district['district_id']}: {district['district_name'][:40]:40s} - {district['barangay_count']} barangays"
        )

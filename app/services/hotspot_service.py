...
MANILA_DISTRICTS = {
    1: {
        ...
        "barangays": list(range(1, 147))
    },
    2: {
        "name": "Tondo (East/Gagalangin)",
        "barangays": list(range(147, 268))
    },
    3: {
...
def record_incident_location(case_id, barangay_number, narrative=None):
    ...
    try:
        ...
        if not district_info:
            return {"status": "error", "message": f"Invalid barangay number: {barangay_number}"}
...
def get_barangay_statistics(cases_data=None):
    ...
    try:
        ...
        stats = {
            ...
            "hotspot_locations": sum(1 for count in barangay_counts.values() if count >= 2),
...
def get_incidents_by_barangay(barangay_number, cases_data=None):
    ...
    try:
        ...
        incidents = [c for c in cases_data if c.get('barangay_number') == barangay_number]
        
        district_info = get_district_from_barangay(barangay_number)
        
        result = {
            "barangay_number": barangay_number,
            "district_name": district_info["district_name"] if district_info else "Unknown",
            "total_incidents": len(incidents),
            "incidents": incidents,
            "retrieved_at": datetime.now().isoformat()
        }
        
        current_app.logger.info(f"Incidents retrieved for barangay {barangay_number}: {len(incidents)}")
...
def save_hotspots_to_db(hotspots_data):
    ...
    try:
        ...
        for hs in hotspots_data:
            ...
            """, (barangay_name, district_name, risk_level, total_cases, resolution_rate))
        ...
        if supabase_sync and supabase_sync.is_ready():
            ...
            try:
                ...
                supabase_sync.client.table('hotspot').delete().neq('Hotspot_ID', -1).execute()
            except Exception as se:
                current_app.logger.warning(f"Could not clear Supabase hotspot table: {se}")
                
            if records_to_sync:
                supabase_sync.batch_sync_to_supabase('hotspot', records_to_sync, 'insert')
            
        return {"status": "success", "message": f"Saved {len(records_to_sync)} hotspots"}

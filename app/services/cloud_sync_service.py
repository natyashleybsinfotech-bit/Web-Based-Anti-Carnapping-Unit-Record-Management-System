        ...
        if operation == "push":
            ...
            try:
                ...
                if users:
                    ...
                    result = supabase_sync.batch_sync_to_supabase('users', user_records, 'insert')
                    ...
                    case_records = [
                        {
                            ...
                            "created_by": row[12],
                            "created_at": row[13].isoformat() if row[13] else datetime.now().isoformat(),
                            "updated_at": row[14].isoformat() if row[14] else datetime.now().isoformat(),
                            "Complainant_ID": row[15]
                        }
                        for row in cases
                    ]
                    result = supabase_sync.batch_sync_to_supabase('cases', case_records, 'insert')
            ...
            try:
                ...
                if logs:
                    ...
                    result = supabase_sync.batch_sync_to_supabase('activity_logs', log_records, 'insert')
                    sync_results["tables_synced"]["activity_logs"] = result.get("synced", 0)
                    sync_results["total_records"] += result.get("synced", 0)
                else:
            ...
            try:
                ...
                if receipts:
                    receipt_records = [
                        {
                            ...
                            "Date_Issued": row[2].isoformat() if row[2] else datetime.now().isoformat(),
                    ...
                    result = supabase_sync.batch_sync_to_supabase('receipts', receipt_records, 'insert')
            ...
            try:
                ...
                if complainants:
                    ...
                    result = supabase_sync.batch_sync_to_supabase('complainants', comp_records, 'insert')
                    sync_results["tables_synced"]["complainants"] = result.get("synced", 0)
                    sync_results["total_records"] += result.get("synced", 0)
                else:
            ...
            try:
                ...
                if hotspots:
                    ...
                    result = supabase_sync.batch_sync_to_supabase('hotspot', hotspot_records, 'insert')
...
def sync_with_cloud_database(data, table, operation="push"):
    ...
    try:
        if operation == "push":
            ...
            if 'id' in data:
                # This is an update
                result = supabase_sync.sync_record_to_supabase(table, data.copy(), 'update')
            else:
                # This is an insert
                result = supabase_sync.sync_record_to_supabase(table, data.copy(), 'insert')

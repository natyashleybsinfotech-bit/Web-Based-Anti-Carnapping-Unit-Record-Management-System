...
def log_activity(user_id, action, description=None, case_id=None, metadata=None):
    ...
    try:
        ...
        current_app.logger.info(f"Activity logged: [{action}] by user {user_id} - {description}")
...
def log_case_activity(user_id, case_id, action, description=None):
    ...
    try:
        ...
        current_app.logger.info(f"Case activity: Case {case_id} - {action} by user {user_id}")
...
def log_login_activity(user_id, username, ip_address, success=True):
    ...
    try:
        ...
        if success:
            ...
            current_app.logger.warning(f"Failed login attempt for {username} from {ip_address}")
...
def log_user_management_activity(admin_id, target_user_id, action, description=None):
    ...
    try:
        ...
        current_app.logger.warning(f"User management: User {target_user_id} was {action} by admin {admin_id}")
...
def log_database_activity(user_id, action, operation_type, description=None):
    ...
    try:
        ...
        current_app.logger.warning(f"Database operation: {operation_type} performed by user {user_id}")
...
def generate_activity_report(start_date, end_date, report_type="summary"):
    ...
    try:
        ...
        current_app.logger.info(f"Activity report generated for period {start_date} to {end_date}")

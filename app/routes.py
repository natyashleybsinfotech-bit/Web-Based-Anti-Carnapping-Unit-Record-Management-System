...
from .services.supabase_realtime_sync import supabase_sync, sync_to_supabase
import MySQLdb
import bcrypt
import re
    ...
    try:
        ...
        if supabase_sync and supabase_sync.is_ready():
            ...
            if created_at_row and created_at_row[0]:
                activity_data["created_at"] = created_at_row[0].isoformat()
            else:
                activity_data["created_at"] = datetime.now().isoformat()
                
            supabase_sync.sync_record_to_supabase('activity_logs', activity_data, 'insert')
...
def login():
    if request.method == "POST":
        ...
        try:
            ...
            if user:
                user_id, full_name, db_username, db_password, role, is_active, force_password_change = user
...
def change_password_required():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            log_activity(user_id, "Password Change", "User changed password on first login")
...
def create_user():
    ...
    try:
        ...
        try:
            ...
            log_activity(session.get("user_id"), "User Created", f"Created new {role}: {username}")
            flash(f"User '{username}' created successfully with default password 'default123'. They must change it on first login.")
...
def admin_dashboard():
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM cases WHERE status='Ongoing'")
        ...
        stats["officer_workload"] = [
            {"name": row[0], "cases": row[1]}
            for row in officer_workload
        ]
...
def officer_dashboard():
    ...
    try:
        ...
        # My case counts
        cur.execute(
            "SELECT COUNT(*) FROM cases WHERE assigned_officer_id=%s",
            (user_id,)
        )
        stats["my_cases"] = cur.fetchone()[0]

        cur.execute(
            "SELECT COUNT(*) FROM cases WHERE assigned_officer_id=%s AND status='Pending'",
            (user_id,)
        )
        stats["my_pending"] = cur.fetchone()[0]

        cur.execute(
            "SELECT COUNT(*) FROM cases WHERE assigned_officer_id=%s AND status='Ongoing'",
            (user_id,)
        )
        stats["my_ongoing"] = cur.fetchone()[0]

        cur.execute(
            "SELECT COUNT(*) FROM cases WHERE assigned_officer_id=%s AND status='Closed'",
            (user_id,)
        )
...
def new_case():
    if request.method == "POST":
        ...
        try:
            ...
            # ── Step 1: Create complainant record (Complainants entity) ──
            ...
            cur.execute(
                ...
                VALUES (%s, %s, %s, %s)
                """,
                (first_name, last_name, data["complainant_contact"], complainant_address)
            )
            complainant_id = cur.lastrowid

            # ── Step 2: Insert case referencing complainant ──
            cur.execute(
                """
                INSERT INTO cases
                (
            ...
            cur.execute(
                ...
                VALUES (%s, %s, %s, %s, %s)
                """,
                (data["reference_no"], data["complainant_email"], case_id, pdf_path, qr_path)
            )
            ...
            if supabase_sync and supabase_sync.is_ready():
                ...
                supabase_sync.sync_record_to_supabase('complainants', complainant_data, 'insert')
                ...
                # Sync receipt (renamed from report_exports)
                receipt_data = {
                ...
                supabase_sync.sync_record_to_supabase('receipts', receipt_data, 'insert')

            # Log activity (automatically syncs to Supabase)
            log_activity(
                session["user_id"],
                "Create Case",
...
def edit_case(reference_no):
    ...
    if request.method == "POST":
        try:
            ...
            assigned_officer_id = request.form["assigned_officer_id"]
            ...
            if supabase_sync and supabase_sync.is_ready():
                ...
                supabase_sync.sync_record_to_supabase('cases', updated_case_data, 'update')

            log_activity(
                session["user_id"],
                "Update Case",
                f"Updated case {reference_no} with status {status}"
            )

            flash(f"Case {reference_no} updated successfully.")
            return redirect(url_for("main.queue"))
    ...
    try:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                mysql.connection.commit()
                log_activity(session.get("user_id"), "User Updated", f"Updated user {user_id}")
...
def delete_user(user_id):
    ...
    try:
        ...
        log_activity(session.get("user_id"), "User Deleted", f"Deleted user: {username}")
...
def toggle_user_status(user_id):
    ...
    try:
        ...
        log_activity(session.get("user_id"), "User Status Changed", f"{status_text} user: {username}")
...
def backup_database():
    ...
    try:
        ...
        lines.append(f"-- Database: {current_app.config.get('MYSQL_DB', 'carnapping_db')}")
        lines.append("")
        ...
        for table in tables:
            ...
            if rows:
                ...
                for row in rows:
                    ...
                    lines.append(f"INSERT INTO `{table}` ({col_names}) VALUES ({values_str});")
            lines.append("")
...
def restore_database(backup_name):
    ...
    try:
        ...
        log_activity(session.get("user_id"), "Database Restore", f"Restored from: {backup_name} ({executed} statements)")
        flash(f"Database restored successfully from {backup_name} ({executed} statements executed).")
        return redirect(url_for("main.manage_backups"))
...
def download_backup(backup_name):
    ...
    try:
        ...
        log_activity(session.get("user_id"), "Backup Downloaded", f"Downloaded backup: {backup_name}")
        return send_file(backup_path, as_attachment=True, download_name=backup_name, mimetype="text/plain")
...
def download_database_sql():
    ...
    try:
                ...
                for row in rows:
                    ...
                    for val in row:
                        if val is None:
                            ...
                            escaped = str(val).replace("\\", "\\\\").replace("'", "\\'").replace("\n", "\\n").replace("\r", "\\r")
                            values.append(f"'{escaped}'")
                    lines.append(f"INSERT INTO `{table}` ({col_names}) VALUES ({', '.join(values)});")
            lines.append("")
        ...
        log_activity(session.get("user_id"), "Database SQL Export", f"Downloaded SQL export: {filename}")
...
def download_database_pdf():
    ...
    try:
        ...
        title_style = ParagraphStyle('Title', parent=styles['Title'], fontSize=18, spaceAfter=6, alignment=TA_CENTER, textColor=colors.HexColor('#003d7a'))
        subtitle_style = ParagraphStyle('Sub', parent=styles['Normal'], fontSize=10, spaceAfter=12, alignment=TA_CENTER, textColor=colors.HexColor('#555555'))
        table_title_style = ParagraphStyle('TableTitle', parent=styles['Heading2'], fontSize=13, spaceBefore=16, spaceAfter=8, textColor=colors.HexColor('#003d7a'))
        ...
        elements.append(Paragraph(f"Generated: {datetime.now().strftime('%B %d, %Y %I:%M %p')} (Manila Time)", subtitle_style))
        ...
        for table_name in tables:
            ...
            visible_indices = [i for i, c in enumerate(col_names) if c not in HIDDEN_COLS]
            visible_cols = [col_names[i] for i in visible_indices]

            elements.append(Paragraph(f"Table: {table_name.upper()}", table_title_style))
            ...
            for row in rows:
                data.append([
                    Paragraph(str(row[i]) if row[i] is not None else "—", cell_style)
                    for i in visible_indices
                ])

            if not rows:
                data.append([Paragraph("<i>No records</i>", cell_style)] + [""] * (len(visible_cols) - 1))
        ...
        log_activity(session.get("user_id"), "Database PDF Export", f"Exported database as PDF: {filename}")
...
def download_database_excel():
    ...
    try:
        ...
        for table_name in tables:
            ...
            for row_idx, data_row in enumerate(rows, start=2):
                ...
                for col_pos, (data_idx, _) in enumerate(visible, start=1):
                    ...
                    cell = ws.cell(row=row_idx, column=col_pos, value=str(val) if val is not None else "")
            ...
            for col_pos, (data_idx, col_name) in enumerate(visible, start=1):
                ...
                ws.column_dimensions[get_column_letter(col_pos)].width = min(max_len + 4, 40)
        ...
        log_activity(session.get("user_id"), "Database Excel Export", f"Exported database as Excel: {filename}")
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, 'w', compression=zipfile.ZIP_DEFLATED) as z:
            for table_name in tables:
                ...
                for row in rows:
                    writer.writerow([row[i] if row[i] is not None else "" for i in visible_indices])
        ...
        log_activity(session.get("user_id"), "Database CSV Export", f"Exported database as CSV ZIP: {filename}")
...
def cloud_sync_status():
    ...
    return render_template("cloud_sync_status.html", sync_status=sync_status, sync_history=sync_history)
...
def queue():
    ...
    try:
        ...
        cur.execute("""
            ...
            ORDER BY
                CASE
                    WHEN c.status IN ('Pending', 'Ongoing') THEN 0
                    ELSE 1
                END,
                c.created_at ASC
        """)
...
def push_all_to_cloud():
    ...
    try:
        ...
        log_activity(session.get("user_id"), "CLOUD_SYNC", f"Manual push to cloud: {result.get('status')}")
...
def pull_all_from_cloud():
    ...
    try:
        ...
        log_activity(session.get("user_id"), "CLOUD_SYNC", f"Manual pull from cloud: {result.get('status')}")
        return json.dumps(result), 200, {"Content-Type": "application/json"}
    except Exception as e:
        return json.dumps({"status": "error", "message": str(e)}), 500, {"Content-Type": "application/json"}
...
def retry_failed_syncs():
    ...
    try:
        ...
        log_activity(session.get("user_id"), "CLOUD_SYNC", f"Retry failed syncs: {result.get('status')}")
        return json.dumps(result), 200, {"Content-Type": "application/json"}
    except Exception as e:
        return json.dumps({"status": "error", "message": str(e)}), 500, {"Content-Type": "application/json"}